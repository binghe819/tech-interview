...
<details>
  ...
  * Redis의 싱글 스레드 특징 -> [참고 - 공식 문서](https://redis.io/topics/latency#single-threaded-nature-of-redis)
    ...
    * 싱글 스레드가 느릴 것으로 판단되지만, 사실은 빠르다. 그 이유는 매 요청을 굉장히 짧은 시간안에 처리하며, 프로세스가 블록 상태가 되지 않도록 설계했기 때문이다.
  * **Redis가 싱글 스레드인 이유**
...
#### Q. Refresh Token은 어디에 저장하는 것이 좋을까요?
<details>
  <summary>답변</summary>
