...
> 
> 혹시 설명을 봐도 모르겠다하시는 분들도 언제든 편하게 이슈 남겨주세요~~ 토론 좋아합니다~~
...
<details>
  ...
  * Enum이란?
    ...
    * **클래스를 이용한 상수**
      * 각각의 상수들은 모두 인스턴스화된 싱글톤 객체이다.
      * 이로 인해 각각의 상수들은 서로 다른 데이터를 의미하게 된다. (참조 변수이므로)
      * 같은 집합의 상수들은 같은 **데이터 타입을 가지므로 서로 다른 집합의 상수끼리 비교 연산은 컴파일 시에 막을 수 있다.**
  * **Enum 사용이유**
    * **상수를 객체처럼 사용할 수 있다. - 추가 속성 + 메시지를 통한 자율성 보장등등 (if문을 줄일 수 있다)**
    * **서로 연관된 상수들끼리 집합으로 묶고, 서로 다른 집합의 상수끼리 비교 연산을 컴파일 타임에 막을 수 있기 때문.**
...
<details>
  ...
  * 제네릭을 사용하지 않을 경우
    * `List`를 사용한다고 가정한다면, 매번 `get`할 때마다 타입 캐스팅을 해주어야 한다. 이때 타입캐스팅이 잘못되면 런타임시 `ClassCastException`이 발생하게 된다. (컴파일 타임때 발견하지 못한다는 것이 가장 큰 문제)
...
<details>
  ...
  * 오버로딩: 재구성
    ...
    * 단, 기존 메서드와 매개변수의 개수 또는 타입이 달라야한다. (메서드 이름과 매개변수는 모두 같은데 반환 타입이 다르면 오류가 발생한다.)
    * 새로운 메서드를 정의하는 것.
  * 오버라이딩: 재정의
...
<details>
  ...
  > 동기화, 비동기화는 각각 두 가지의 의미로 사용되는 듯 하다.

  * 동기화
    1. **프로세스 또는 스레드들이 동시에 실행되는 시점에 서로 작업에 끼어들지 못하도록 보장하는 것. 부수효과가 발생하지 않는 것.** (Lock처리)
    2. **어느 메서드가 실행하는 동안 다른 메서드를 실행이 불가능하게 블록하는 것.**
      * 작업을 요청한 후 해당 작업의 결과가 나올 때까지 기다린 후 처리하는 것.
      * 메서드를 실행시킴과 동시에 반환 값이 기대되는 경우.
  * 비동기화
    1. 어느 메서드를 실행하는 도중에도 다시 메서드를 실행이 가능하다. (멀티 스레드)
...
<details>
  ...
  * Thread-Safe란 멀티 스레드환경에서 어떤 함수나 변수, 혹은 객체가 **여러 스레드로부터 동시에 접근이 이루어져도 프로그램의 실행에 문제가 없음**을 의미한다.
    * **다른 스레드로 인해 부수효과가 발생하지 않는 것을 의미**
    * **두 개의 스레드가 동시에 데이터에 접근하여 변경한다면 계산 결과가 덮여씌워지기때문에 Thread-Safe하게 만들어줘야한다.**
  * Thread-Safe 예시
    * `synchronized`붙은 메서드, `ConcurrentHashMap`등등
  * Thread-Safe하게 만들기 위해선
    * Mutual Exclusion
      * Thread에 Lock이나 Semaphore를 걸어서 공유자원에는 하나의 thread만 접근 가능하게 만든다.
    * Thread-Local
      * 각 스레드에서만 접근 가능한 저장소를 사용하여 동시 접근을 막는다.
    * 불변 객체
      * 객체 생성 이후에 값을 변경할 수 없도록 만든다. (변경시 방어적 복사를 통해 새로운 값을 만드는 것.)
  
  --- 
</details>

#### Q. 빌더 패턴은 thread-safe한가? - 예정
<details>
  <summary>답변</summary>
...
<details>
  ...
  * 8과 비교하여 11에서 변경된 점
    1. 편의 메서드가 많이 생겼다.
       * `isBlank()`, `repeat()`...등등
       * `List.of`도 생김.
    2. var 사용가능해짐 (동적 타입체크)
    3. `java.nio.file.Files` 클래스 유틸 메서드 추가됨
    4. JDK 11의 Long-Term-Support(LTS)가 더 길다..
...
<details>
  ...
  * 응집도
    ...
    * **응집도 높음** (좋음): 모듈 내의 요소들이 하나의 목적을 위해 긴밀하게 협력한다면 해당 모듈은 높은 응집도를 가진다고 할 수 있다.
...
<details>
  ...
  * **비침투성 - POJO** (관심사 분리) - 객체지향적 특징
    ...
    * ex. 우아한 테크코스 체스 미션 -> 처음엔 콘솔에서만 동작하도록 했음. -> 웹 서비스를 위해 스프링을 적용시키면서 기존의 도메인 코드와 서비스 관련 로직의 변경이 발생하지 않음. (재활용)
...
<details>
  ...
  * 전파 레벨
    * 진행되고 있는 트랜잭션에서 다른 트랜잭션이 호출될 때 어떻게 처리할지 정하는 것.
  * 잘 설명된 블로그: https://deveric.tistory.com/86
  
  --- 
</details>

#### Q. @Transactional vs DB 트랜잭션 - 예정
<details>
  <summary>답변</summary>
...
<details>
  ...
  * **Connection Pool란**
    * **DB와 연결된 Connection을 미리 만들어서 Pool 속에 저장해두고 있다가 필요할 때 Connection을 Pool에서 쓰고 다시 Pool에 반환하는 기법.**
    * 동시 접속자 가질 수 있는 Connection을 하나로 모아놓고 관리한다. (미리 Connection 객체를 생성하고 해당 Connection 객체를 관리하는 것을 의미)
  * 왜 사용하나? (Connection Pool이 없다면)
    * Connection Pool이 없다면 매 요청마다 DB와의 새로운 연결을 생성해야한다. 이는 매번 DB 인증을 위해 사용자 이름, 암호, TSL 사양이 필요하며 연결 수가 증가함에 따라 비용이 커지게 된다.
    * 또한, 생성될 때마다 메모리가 필요하며, 만약 연결 수를 제약두지 않으면 메모리가 부족한 상황이 발생할 수 있다.
    * 쉽게 말해, **Connection Pool을 이용하면 정해진 개수만큼 미리 연결된 상태의 스레드 풀을 이용하여 연결 비용과 메모리 부족 문제를 해결할 수 있다.**
  
  --- 
</details>

#### Q. Connection Pool의 동작 (DB에 접근하는 단계)
<details>
  <summary>답변</summary>
  
  --- 

  <p align="center"><img src="./image/connection_pool.png" ><Br>출처: 자바 웹을 다루는 기술 </p>
  
  * 애플리케이션 실행
  * DB와 연결된 Connection 객체들을 미리 생성하여 Pool에 저장.
    * 이때 중요한 점은 미리 DB와 Connection을 맺어놓고있다는 점이다.
  * DB에 요청시, pool에서 Connection 객체를 가져와 DB에 접근한다.
  * 처리가 끝나면 다시 Pool에 반납한다.

  > 이는 굉장히 추상적으로 작성된 글이다. 추후에 더 자세히 정리할 예정이다.

  --- 
</details>

#### Q. Connection Pool이 부족한 경우 발생할 수 있는 문제
<details>
  <summary>답변</summary>
...
#### Q. WAS의 Connection Pool와 DB Connection Pool의 개수
<details>
  <summary>답변</summary>
  
  --- 
  
  * **Connection Pool 개수는 트레이드 오프이다.**
    * Connection은 메모리와 관련이 있다. 높게 측정하면 메모리를 많이 사용하지만, 커넥션은 많이 연결할 수 있다.
    * 반대로, 낮게 측정하면 메모리는 적게 사용하지만, 많은 요청을 처리하지 못하고 대기 할 수 있다.
  * 어느 것을 더 높게 설정해야하는가?
    * WAS의 Connection이 더 많아야하는 이유는 **WAS로 오는 모든 요청이 DB를 사용하는 요청만 존재하는건 아니기 때문이다.**
    * 그러므로, DB Connection Pool보다 WAS의 Connection Pool이 많아야한다.
...
<details>
  ...
  * 개인적으론 비즈니스 코드를 객체지향적으로 작성할 수 있다는 장점때문에 사용했다.
    1. JPA에서 제공하는 Repository는 도메인 계층에 속하며, Collection처럼 사용된다. 그러므로 정말 Collection 다루듯이 사용하면 JPA가 자동으로 영속화를 시켜준다. 또한, 더티 체킹도 해준다.
    2. 객체 그래프 탐색도 좋은 이유인 듯하다. SQL Mapper를 사용하면 처음 실행하는 SQL에 따라 객체 그래프를 어디까지 탐색할 수 있는지 정해야하기 때문에 유연하지 못하다. 하지만 JPA는 지연 로딩을 통해 필요할 때 데이터를 불러와주기 때문에 유용하다.
...
<details>
  ...
  * 한 줄 정리
    * 먼저 클라이언트와 서버가 통신에 사용할 대칭키를 비대칭키 암호화 방식을 이용해 안전하게 주고 받고 주고받은 키를 가지고 대칭키 암호화 방식으로 통신한다.
  * HTTPS 추상적인 동작 과정
    * 
  * HTTPS 추상적인 구체 과정 (SSL Handshake)
    * 
  
  --- 
</details>

#### Q. 포워드 프록시와 리버스 프록시의 차이
<details>
  <summary>답변</summary>
...
<details>
  ...
  * 트랜잭션이란?
    * **복수 쿼리를 독립적으로 한 단위로 묶는 것**. 더 이상 나눌 수 없는 단위 작업.
  ...
  * 트랜잭션 성질 (ACID)
    ...
    * **Isolation (독립성) -> 병행 제어**
      * 데이터 조작을 복수의 사용자가 동시에 실행해도 '각각의 처리가 모순없이 실행되는 것을 보증'하는 것.
...
<details>
  ...
  * 주요 지표
    * type: 접근 방식 -> 테이블에서 어떻게 행 데이터를 가져올 것인지를 의미.
    * key: 실제로 옵티마이저가 사용한 인덱스.
    * rows: 몇 행을 가져왔는가를 표시 -> 평균 몇 개의 행을 접근했는지 표시. (실제 행 수와 반드시 일치하진 않는다)
    * filtered: 행 데이터를 가져와 필터(조건)를 통해 몇 행이 남는지 표시한다. 
    * extra: 옵티마이저가 동작하는 힌트
      * 중요 키워드
        * Using filesort(ORDER BY를 인덱스로 해결하지 못하고, 결과 직접 quick sort로 정렬하는 것.)
...
#### Q. DB Replication 마스터 DB Failover - 예정
<details>
  <summary>답변</summary>
  
  --- 
  
  * MHA (Master High Availability) 사용
    * Master DB 장애 발생시, 자동으로 failover를 수행하여 Slave DB를 Master DB로 승격시켜 서비스 다운타임을 최소화하는 솔루션이다.
